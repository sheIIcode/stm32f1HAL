...
void ST7735_DrawWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h,const uint8_t* dataPoints)
{
    // clipping
    if((x >= _width) || (y >= _height)) return;
    if((x + w - 1) >= _width) w = _width - x;
...
//    uint8_t data[] = { color >> 8, color & 0xFF };
    ...
    for(x = 1; x < w; x++)
    {
        for(y = 0; y < h; y++)
        {
        	if(dataPoints[x] == y || dataPoints[x-1] == y )
        		HAL_SPI_Transmit(&ST7735_SPI_PORT, 0xFF, 2, HAL_MAX_DELAY);
        	else
        		HAL_SPI_Transmit(&ST7735_SPI_PORT, 1, 2, HAL_MAX_DELAY);
        }
    }
    TFT_CS_H();
